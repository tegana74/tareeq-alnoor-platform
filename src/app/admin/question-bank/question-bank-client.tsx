...
import { CheckCircle2, ChevronDown, ChevronUp, HelpCircle, Loader2, Plus, Save, Trash2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import {
  createBankChapterAction,
  deleteBankChapterAction,
...
export function ChapterForm({ subjects }: { subjects: { id: string; name: string }[] }) {
  const { state, formAction, pending } = useSubmit(createBankChapterAction)
  const [open, setOpen] = useState(false)

  return (
    <div>
      <Button type="button" variant="navy" size="sm" onClick={() => setOpen(!open)}>
        <Plus className="h-4 w-4" /> فصل جديد
      </Button>
      {open && (
        <form action={formAction} className="mt-3 rounded-xl border border-slate-200 bg-slate-50 p-4">
          <div className="grid gap-2 sm:grid-cols-2">
            ...
            <input name="name" required placeholder="اسم الفصل" className={inputCls} />
          </div>
...
export function ChapterActions({ chapterId }: { chapterId: string }) {
  const { formAction, pending } = useSubmit(deleteBankChapterAction)
  return (
    <form action={formAction} onSubmit={() => confirm("حذف الفصل وجميع أسئلته؟")}>
